$(function(){
    ...
    if (_token) {
        try {
            let wss = new WebSocket(`wss://Award-API.wncn7n27lcmtx2x.repl.co/v1/gateway?u=1&t=${_token}`);

            setInterval(() => {
                if (wss.readyState == WebSocket.CLOSED) {
                    wss = new WebSocket(`wss://Award-API.wncn7n27lcmtx2x.repl.co/v1/gateway?u=1&t=${_token}`);
                };
            }, 5000);
